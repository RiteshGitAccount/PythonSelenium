...
class HomePageFunctions(BaseClass):
    ...
    def search_product(self, product):
        self.send_text(self.hm_obj._search_input, product)
        self.hit_enter()
    ...
    def enter_login_username(self, login_user_detail):
        ...
        if self.wait_for_element_visibility(self.hm_obj._close_login_window_btn):
            if login_user_detail != "":
                self.send_text(self.hm_obj._username_input, login_user_detail)
    ...
    def close_login_window(self):
        if self.wait_for_element_visibility(self.hm_obj._close_login_window_btn):
            self.click_element(self.hm_obj._close_login_window_btn)
            self.print_log("Login window closed")
        self.wait_for_element_invisibility(self.hm_obj._close_login_window_btn)

    def select_product(self, product):
        self.click_element(self.hm_obj._product_link.format(product))
        self.print_log(f"{product=} selected")

    def add_to_cart(self):
        self.click_element(self.hm_obj._add_to_cart)
